...
class ResetTeamCaptainsScreen extends StatefulWidget {
  const ResetTeamCaptainsScreen({super.key});

  @override
  State<ResetTeamCaptainsScreen> createState() => _ResetTeamCaptainsScreenState();
}

class _ResetTeamCaptainsScreenState extends State<ResetTeamCaptainsScreen> {
            ...
            children: [
              ...
              Expanded(
                  child: Container(
                    ...
                    child: BlocConsumer<RecreateTeamBloc, RecreateTeamState>(
                        builder: (_, state) {
                          return buildInitialInput(data: data);
                        }, listener: (_, state) {
                      if (state is RecreateTeamLoadingState) {
                        isLoading = true;
                        setState(() {});
                      } else if (state is RecreateTeamSuccessfulState) {
                        isLoading = false;
                        setState(() {});
                      ...
                      } else if (state is RecreateTeamFailedState) {
                        isLoading = false;
                        setState(() {});
                          ...
                          onPressed: (data.isViceCaptainSelected && data.isCaptainSelected) ? (){
                            if(!isLoading){
                              if (data.isViceCaptainSelected &&
                                ...
                                final recreateTeam = BlocProvider.of<RecreateTeamBloc>(context);
                                recreateTeam.add(PatchRecreateTeamEvent(players));
